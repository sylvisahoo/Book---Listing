...
class _BookDetailScreenState extends ConsumerState<BookDetailScreen> {
  ...
  Future<void> _loadBookDetails() async {
    ...
    final book = await ref.read(bookNotifierProvider.notifier).getBookDetails(_bookId!);
    ...
    if (mounted) {
      setState(() {
        ...
        _isLoading = false;
        if (book == null) {
          _error = ref.read(bookNotifierProvider).errorMessage ?? 'Failed to load details.';
        }
      });
    }
  }
  ...
  Future<void> _confirmDelete() async {
    ...
    if (confirmed == true && mounted) {
      final success = await ref.read(bookNotifierProvider.notifier).deleteBook(_book!.id);
      if (success && mounted) {
                ...
                return ListTile(
                  ...
                  onTap: isCurrent
                      ...
                      : () async {
                          ...
                          final success = await ref.read(bookNotifierProvider.notifier)
                  ...
                  onPressed: () async {
                    if (formKey.currentState!.validate()) {
                      if (selectedDate.isAfter(DateTime.now())) {
                        setStateDialog(() {
                          localError = 'Completion date cannot be in the future';
                        });
                        return;
                      }
                      ...
                      final success = await ref.read(bookNotifierProvider.notifier).addReview(
                        book.id,
                        dateController.text,
                        selectedRating,
