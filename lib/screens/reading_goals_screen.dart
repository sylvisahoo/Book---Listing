...
class ReadingGoalsScreen extends StatefulWidget {
  ...
  const ReadingGoalsScreen({super.key, required this.bookProvider});
...
class _ReadingGoalsScreenState extends State<ReadingGoalsScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      widget.bookProvider.fetchGoals();
    });
  }
                ...
                ElevatedButton(
                  ...
                  onPressed: () async {
                    if (formKey.currentState!.validate()) {
                      ...
                      bool success;
                      if (isEditing) {
                        success = await widget.bookProvider.updateGoal(
                          existingGoal['id'] as int,
                          target,
                        );
                      } else {
                        success = await widget.bookProvider.createGoal(
                          target,
                          year,
                        );
                      }
    ...
    return Scaffold(
      ...
      body: Consumer<BookProvider>(
        builder: (context, provider, child) {
          if (provider.isGoalsLoading && provider.goals.isEmpty) {
            return const Center(
      ...
      floatingActionButton: Consumer<BookProvider>(
        builder: (context, provider, child) {
          if (provider.goals.isEmpty) return const SizedBox();
